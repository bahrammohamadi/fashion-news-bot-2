...
# 1. Update _find_best_candidate definition and sorting/throttling block
old_find_def_block = """async def _find_best_candidate(
    ...
    if PRODUCT_FIRST:
        ...
        log_fn(
            f"[feed] Core-Product-first ON: {n_core} core apparel / {n_products} total product candidates / "
        ...
        if n_products == 0:
            ...
            try:
                ...
                cutoff_news_hours = 12  # Don't post news if another news was posted in last 12 hours
...
new_find_def_block = """async def _find_best_candidate(
    feeds, databases, database_id, collection_id,
    time_threshold, sdk_mode, schema, now,
...
if old_find_def_block in text:
    text = text.replace(old_find_def_block, new_find_def_block)
else:
    print("Warning: old_find_def_block not matched.")
...
print("Find best candidate logic applied successfully.")
